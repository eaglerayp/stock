...
def getRDPDatas(raw_datas, diffDay):
    n = len(raw_datas)
    print("Data count:", n)
    diff_datas = []
    # diffDay mins R+n, pre-process PCA target data
    ...
    print("n stocks:", len(diff_datas[0]))
    return diff_datas
...
STOCK_COLLECTION = MONGO_CLIENT.stock["TWSE_CON"]
query_result = STOCK_COLLECTION.find(
    filter={'Date': {'$gt': 20120000}},
    # limit=2000,
...
diffDay = 1
test_samples = 60
diff_datas = getRDPDatas(raw_datas, diffDay)
answers = []
...
for x in pca_components:
    ratio = ratio + x
print("PCA component:", components)
